    ...
    return (
      <div className='d-flex justify-content-center' style={{ paddingBottom: this.props.smDown ? 30 : 58, paddingTop: this.props.smDown ? 4 : 10 }}>
        <div className='d-flex align-items-center search' style={{ backgroundColor: 'white' }}>
          ...
          />
          <svg style={{cursor : 'pointer'}} onClick={() => { this.setState({ inputValue: '' }); this.search(1) }} width="1.5em" height="1.5em" viewBox="0 0 16 16" className={`bi bi-x-square-fill ${this.state.inputValue.length ? 'visible' : 'invisible'}`} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
